...
uv run main.py --query "Fibonacci数列的第1028项 (F(0) = 0, F(1) = 1), 使用快速幂算法来求解，并返回求解的结果"

# uv run main.py --query "假如你是一个专业的软件测试工程师，现在我邀请你测试代码沙盒运用，包括（环境下载、命令行执行、python代码执行、列出工作区所有文件）这一套工具都已经集成在了Coder中，请帮我针对前面描述的功能且覆盖面广的测试，并且生成详细的报告(沙盒默认是linux系统，python 是3.11，基本没有预装的开发工具；资源限制256MB一个核心，通过远端服务请求，可以下载外部依赖，可以在 沙盒内部随便 创建销毁文件或者文件夹； 关注功能正确性与性能表现，安全性暂时不考虑；无， 唯一的要求是尽可能详细的测试所有的功能，预期测试时间：10分钟)"


##沙盒默认是linux系统，python 是3.11，基本没有预装的开发工具；资源限制256MB一个核心，通过远端服务请求，可以下载外部依赖，可以在 沙盒内部随便 创建销毁文件或者文件夹； 关注功能正确性与性能表现，安全性暂时不考虑；无， 唯一的要求是尽可能详细的测试所有的功能，预期测试时间：10分钟