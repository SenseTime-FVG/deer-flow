...
2.当任务完整度达到0.9以上，才可调用planner tool向规划者传递任务
3.确保task json中的每个字段都有实质内容，避免填充无意义的占位符
4.planner tool中的requirements和constraints中不要包含具体的工具/API描述，这会限制后续模型的发挥。例如：'数据需来自可靠气象API'-> '气象数据需要有可靠的来源'`
