...
class SupervisorNode(BaseNode):
    ...
    def __init__(self, toolmanager):
        super().__init__(
            "supervisor", AgentConfiguration.NODE_CONFIGS["supervisor"], toolmanager
        )
    ...
    async def execute(self, state: Dict[str, Any], config: RunnableConfig) -> (
        ...
        supervisor_input = apply_prompt_template(
            "supervisor", supervisor_state, configurable
        )
                ...
                if action == "advise":
                    ...
                    return Command(
                        update={
                            "messages": [
                                HumanMessage(
                                    content=f"Step rejected: {suggestion}. \nStep score: {score}.\nPlease retry.",
                                    name="supervisor",
                                )
                            ],
                    ...
                    if next_action:
                        ...
                        self.update_plan_action_status(
                            self.current_plan,
                            current_step_index,
                        ...
                        next_node = AgentConfiguration.STEP_TYPE_TO_NODE[
                            next_action.type.lower()
                        ]
                        # .get(
                        ...
                        next_step_summary = self.get_action_with_dependencies_json(
                            self.current_plan,
                            next_action.id,
                            state.get("resources", []),
                        )
                        ...
                        return Command(
                            update={
                                "messages": [
                                    ...
                                    HumanMessage(
                                        content=next_step_summary, name="supervisor"
                                    ),
                                ],
